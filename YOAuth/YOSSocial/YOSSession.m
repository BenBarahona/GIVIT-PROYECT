...
- (id)initWithConsumer:(YOAuthConsumer *)aConsumer andApplicationId:(NSString *)anApplicationId
{
	if(self = [self init])
	{
        ...
        NSLog(@"Token Store is %@",sessionTokenStore);
...
- (void)saveSession
{
	if(self.tokenStore) 
	{
        NSLog(@"in save session Access token is %@",self.accessToken.key);
          NSLog(@"in save session Access token is %@",self.accessToken.secret);
		...
		if(self.requestToken) {
			...
			;//[tokenStore removeRequestToken]; //ARIJIT
		}	
	}
}
...
{	
   ...
   appDelegate.appdelegateRequestToken=tokenAuthRequest;
    
    NSLog(@"Token Auth request is %@",tokenAuthRequest);
    
     NSLog(@"Token Auth appDelegate.appdelegateRequestToken is %@",appDelegate.appdelegateRequestToken);
	...
	if(self.requestToken)
    {
        ...
        NSLog(@"resumeSession New Access token is %@",newAccessToken.key);
         NSLog(@"resumeSession New Access token is %@",newAccessToken.secret);
...
- (void)sendUserToAuthorizationWithCallbackUrl:(NSString *)callbackUrl
{
   ...
   // appDelegate=(SocialSampleAppDelegate *)[[UIApplication sharedApplication]delegate];
	...
	if(newRequestToken && newRequestToken.key && newRequestToken.secret) {
        ...
        NSLog(@"Authorization URL is %@",authorizationUrl);
        
		//NSLog(@"authorizationUrl: %@",[authorizationUrl absoluteString]);
        
        appDelegate=(GIM_AppDelegate*)[[UIApplication sharedApplication]delegate];
        
        appDelegate.authorizationUrl=authorizationUrl;
        

        NSLog(@"appDelegate.authorizationUrl URL is %@",appDelegate.authorizationUrl);
