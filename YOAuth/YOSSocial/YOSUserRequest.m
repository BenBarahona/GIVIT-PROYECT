...
// http://social.yahooapis.com/v1/user/userGUID/contacts?format=json

- (BOOL)fetchUserContacts:(id)delegate
{
	NSString *method = [NSString stringWithFormat:@"contacts"];
    
    
    NSLog(@"-------self.baseUrl is %@",self.baseUrl);
    NSLog(@"-------self.apiVersion is %@",self.apiVersion);
    NSLog(@"-------self.user.guid is %@",self.user.guid);
